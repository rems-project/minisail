...
lemma subtype_trans: 
  ...
  shows "\<Theta> ; \<B> ; \<Gamma> \<turnstile> \<tau>1 \<lesssim> \<tau>3"
proof -
  obtain y::x where yf:"atom y \<sharp> (\<Gamma>,  \<tau>1 , \<tau>2, \<tau>3 )" using obtain_fresh  by metis

  obtain c1 and b1 where t1:"\<tau>1 = \<lbrace> y : b1 | c1 \<rbrace>" using obtain_for_t_with_fresh yf by force
  obtain c2 and b2 where t2:"\<tau>2 = \<lbrace> y : b2 | c2 \<rbrace>" using obtain_for_t_with_fresh yf by force
  obtain c3 and b3 where t3:"\<tau>3 = \<lbrace> y : b3 | c3 \<rbrace>" using obtain_for_t_with_fresh yf by force

  obtain x::x where xf:"atom x \<sharp> (\<Gamma>,y,c1,c2,c3,\<Theta>, \<B>, \<Gamma>, y,  c1 , c3)" using obtain_fresh by metis
  have beq: "b1 = b2 \<and> b2 = b3" using assms subtype_eq_base t1 t2 t3 by simp

  have vld1: "\<Theta> ; \<B> ; ((x, b1, c1[y::=V_var x]\<^sub>v) #\<^sub>\<Gamma> \<Gamma>) \<Turnstile> c2[y::=V_var x]\<^sub>v" using subtype_valid fresh_prod5 assms t1 t2 xf beq by simp
  have vld2: "\<Theta> ; \<B> ; ((x, b1, c2[y::=V_var x]\<^sub>v) #\<^sub>\<Gamma> \<Gamma>) \<Turnstile> c3[y::=V_var x]\<^sub>v" using subtype_valid fresh_prod5 assms t3 t2 xf beq by simp
  thm valid_trans[where z=x and v="V_var x"]
  have "\<Theta> ; \<B> ; ((x, b1, c1[y::=V_var x]\<^sub>v) #\<^sub>\<Gamma> \<Gamma>) \<Turnstile> c3[y::=V_var x]\<^sub>v" using valid_trans_2 vld1 vld2 by metis

  moreover have "\<Theta> ; \<B> ; \<Gamma> \<turnstile>\<^sub>w\<^sub>f \<lbrace> y : b1 | c1 \<rbrace>" using t1 subtype_wfT assms by simp 
  moreover have "\<Theta> ; \<B> ; \<Gamma> \<turnstile>\<^sub>w\<^sub>f \<lbrace> y : b1 | c3 \<rbrace>" using t3 beq subtype_wfT assms by simp 
  moreover have "atom x \<sharp> (\<Theta>, \<B>, \<Gamma>, y , c1 , y , c3)" using xf fresh_prod5 fresh_prod10 by simp
...
lemma subtype_eq_e_nil:
  ...
  using  assms fresh_def e.fresh  supp_GNil apply metis+
  done



(* FIXME use valid_fst and subtype_baseI. Maybe not. Too many premises needed for latter *)
...
proof -  
  ...
  proof(rule  subtype_base_fresh)
    ...
    show  "wfT \<Theta>  \<B>  \<Gamma> (\<lbrace> x : b  | CE_val (V_var x)  ==  CE_op opp [(V_lit (L_num n1))]\<^sup>c\<^sup>e [(V_lit (L_num n2))]\<^sup>c\<^sup>e  \<rbrace>)" (is "wfT \<Theta> \<B>  ?A ?C")   
    ...
    show   "\<Theta>; \<B> ; (x, b, (CE_val (V_var x)  ==  CE_val (V_lit (ll)) )) #\<^sub>\<Gamma> \<Gamma>  
      ...
      using valid_bop assms xf by simp
...
lemma infer_bool_options2:
  ...
  using assms
proof(nominal_induct v  arbitrary: b rule: v.strong_induct)
  case (V_lit l)
  then show ?case proof(nominal_induct l  rule: l.strong_induct)
    case (L_num nat)
    hence "\<turnstile> L_num nat  \<Rightarrow> \<lbrace> z : b | c \<rbrace>" using infer_v_elims(2)   by (metis (no_types, lifting))
    hence "b = B_int" using infer_l_elims(3) type_for_lit.simps(3)   by (metis \<tau>.eq_iff)
    then show ?case using L_num by fastforce
  next
    case L_true
    hence "\<turnstile> L_true  \<Rightarrow> \<lbrace> z : b | c \<rbrace>" using infer_v_elims(2)   by (metis (no_types, lifting))
    hence "b = B_bool"  using infer_l_elims type_for_lit.simps   by (metis \<tau>.eq_iff)
    then show ?case by blast
  next
    case L_false
    hence "\<turnstile> L_false  \<Rightarrow> \<lbrace> z : b | c \<rbrace>" using infer_v_elims(2)   by (metis (no_types, lifting))
    hence "b = B_bool"  using infer_l_elims type_for_lit.simps   by (metis \<tau>.eq_iff)
    then show ?case by blast
  next
    case L_unit
    hence "\<turnstile> L_unit  \<Rightarrow> \<lbrace> z : b | c \<rbrace>" using infer_v_elims(2)   by (metis (no_types, lifting))
    hence "b = B_unit" using infer_l_elims type_for_lit.simps   by (metis \<tau>.eq_iff)
    then show ?case using L_unit by fastforce
  next
    case (L_bitvec x)
    hence "\<turnstile> L_bitvec x  \<Rightarrow> \<lbrace> z : b | c \<rbrace>" using infer_v_elims   by (metis (no_types, lifting))
    hence "b = B_bitvec" using infer_l_elims type_for_lit.simps   by (metis \<tau>.eq_iff)
    then show ?case using L_bitvec by fastforce
  qed
next
  case (V_var x) 
  then show ?case using v.supp V_var supp_at_base[of x] by auto
next
  case (V_pair v1 v2)
  then show ?case using infer_v.simps 
     \<tau>.eq_iff infer_v_elims  by (metis b.distinct)
next
  case (V_cons dc v)
  then show ?case using infer_v.simps 
     \<tau>.eq_iff infer_v_elims  by (metis b.distinct)
...
proof(nominal_induct v  rule: v.strong_induct)
  ...
  next
    case L_unit
    hence "\<turnstile> L_unit  \<Rightarrow> \<lbrace> z : b | c \<rbrace>" using infer_v_elims   by (metis (no_types, lifting))
    hence "b = B_unit"  using infer_l_elims type_for_lit.simps   by (metis \<tau>.eq_iff)
    then show ?case by simp
  next
    case (L_bitvec x)
...
next
  case (V_var x)
  then show ?case using infer_v_elims V_var  
    by (metis finite.emptyI fresh_atom_at_base fresh_finite_insert v.fresh(2))
next
  case (V_pair v1 v2)
...
next
  ...
  from V_consp(2) show ?case proof(nominal_induct "V_consp s dc bc v" \<tau> rule:infer_v.strong_induct)
   case (infer_v_conspI bv dclist \<Theta> tc \<B> \<Gamma> tv z)
    moreover hence "atom z \<sharp> (V_consp s dc bc v)" unfolding v.fresh using pure_fresh fresh_prodN * by metis
    ultimately show ?case using fresh_prodN by metis
  qed
qed
...
(*
  ...
  obtain z and b where "\<turnstile> l \<Rightarrow> (\<lbrace> z : b  | CE_val (V_var z)  ==  CE_val (V_lit l)  \<rbrace>)" 
    using infer_l_form2 assms infer_v_wf  by metis
  ...
  qed
  have "b' = b" 
    using V_lit.prems(1) \<open> \<turnstile> l \<Rightarrow> \<lbrace> z : b | CE_val (V_var z) == CE_val (V_lit l) \<rbrace>\<close> \<tau>.eq_iff infer_l_uniqueness zbc1
    by (meson infer_v_elims(2))
...
next
  ...
  hence "\<Theta>;  \<B> ; \<Gamma>' \<turnstile> V_var x \<Rightarrow> (\<lbrace> z' : b  | CE_val (V_var z')  ==  CE_val (V_var x)  \<rbrace>)" 
    using infer_v_varI V_var ** z' by simp
...
next
  ...
  proof(nominal_induct "V_consp s dc b v" \<tau> avoiding: \<Gamma>' rule: infer_v.strong_induct)
  case (infer_v_conspI bv dclist \<Theta> tc \<B> \<Gamma> tv z)
    show ?case proof
      show \<open>AF_typedef_poly s bv dclist \<in> set \<Theta>\<close> using infer_v_conspI by auto
      show \<open>(dc, tc) \<in> set dclist\<close> using infer_v_conspI by auto
      show \<open> \<Theta> ; \<B> ; \<Gamma>' \<turnstile> v \<Rightarrow> tv\<close> using infer_v_conspI  by metis
      show \<open>\<Theta> ; \<B> ; \<Gamma>'  \<turnstile> tv \<lesssim> tc[bv::=b]\<^sub>\<tau>\<^sub>b\<close> using infer_v_conspI subtype_weakening by metis
      show \<open>atom z \<sharp> (\<Theta>, \<B>, \<Gamma>', v, b)\<close> using infer_v_conspI by auto
      show \<open>atom bv \<sharp> (\<Theta>, \<B>, \<Gamma>', v, b)\<close> using infer_v_conspI by auto
      show \<open> \<Theta> ; \<B>  \<turnstile>\<^sub>w\<^sub>f b \<close> using infer_v_conspI by auto
   qed
...
next
  ...
  moreover hence t:"\<tau> = (\<lbrace> z'' : B_id s  | CE_val (V_var z'')  ==  CE_val (V_cons s dc v)  \<rbrace>)" proof -
    have "atom z'' \<sharp> AE_val (V_cons s dc v)" using zdash e.fresh v.fresh  Un_commute b.supp(3) fresh_def 
            sup_bot.right_neutral supp_b_empty v.supp(4) by metis
    moreover have "atom z \<sharp> AE_val (V_cons s dc v)" using * e.fresh v.fresh  Un_commute b.supp(3) fresh_def 
            sup_bot.right_neutral supp_b_empty v.supp(4) by metis
