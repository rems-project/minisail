...
next
  ...
  show ?case unfolding \<tau>.eq_iff proof
    ...
    show "[[atom z']]lst. c'[bv1'::=b1']\<^sub>c\<^sub>b = [[atom z]]lst. c[bv1::=b1]\<^sub>c\<^sub>b" proof(subst Abs1_eq_iff_all(3),rule,rule,rule)
      fix ca::x
  ...
  done
{
  ...
  moreover have  "atom y \<sharp> (bva, ba) \<and> atom ya \<sharp> (bva, ba)" using x_fresh_b x_fresh_bv fresh_Pair by simp  
  ultimately show ?case 
...
next
  ...
  moreover have  "atom u \<sharp> (bva, ba) \<and> atom ua \<sharp> (bva, ba)" using x_fresh_b x_fresh_bv   fresh_Pair by simp
  ultimately show ?case using eqvt_triple eqvt_at_proj by metis
next
  case (4 x1 s' x1a s'a bva ba c)
  moreover have  "atom x1 \<sharp> (bva, ba) \<and> atom x1a \<sharp> (bva, ba)" using x_fresh_b x_fresh_bv   fresh_Pair by simp
  ultimately show ?case using eqvt_triple eqvt_at_proj by metis
}
qed
...
(* Note: Using just simp as second apply unpacks and gives a single goal, 
auto gives 43 non-intuitive goals but that be easier to solve but tedious but can be used
to find what is missing in fact in this case I saw that one of the goals was going through with metis and the next
wasn't. turns out the defn of the subst was wrong *)
...
proof(goal_cases)
  case (1 P a1 a2 b)
  then show ?case using   \<Gamma>.exhaust neq_GNil_conv by force
qed
nominal_termination (eqvt) by lexicographic_order
...
instance proof
  ...
  next
    ...
    thm  fresh_subst_if[of j b' i x]
    have 1:"j \<sharp> b'[i::=x]\<^sub>b\<^sub>b = (atom i \<sharp> b' \<and> j \<sharp> b' \<or> j \<sharp> x \<and> (j \<sharp> b' \<or> j = atom i))" using fresh_subst_if[of j b' i x] subst_b_b_def by auto
    have 2:" j \<sharp> c'[i::=x]\<^sub>c\<^sub>b = (atom i \<sharp> c' \<and> j \<sharp> c' \<or> j \<sharp> x \<and> (j \<sharp> c' \<or> j = atom i))" sory
    have 3:"j \<sharp> \<Gamma>'[i::=x]\<^sub>\<Gamma>\<^sub>b = (atom i \<sharp> \<Gamma>' \<and> j \<sharp> \<Gamma>' \<or> j \<sharp> x \<and> (j \<sharp> \<Gamma>' \<or> j = atom i))" sory  
    show ?case unfolding  subst_gb.simps subst_b_\<Gamma>_def fresh_GCons fresh_prod3 1 2 3 * fresh_GCons fresh_prod3 *)
...
ML \<open>Ctr_Sugar.ctr_sugar_of @{context} @{type_name b} |> Option.map #ctrs\<close>
